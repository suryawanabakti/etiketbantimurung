            ...
            {tikets.map((tiket) => (
                <Card
                    ...
                    className={`group cursor-pointer rounded-[32px] md:rounded-[40px] transition-all duration-500 border-2 overflow-hidden ${selectedTiket?.id === tiket.id
                        ? 'border-rose-500 bg-rose-50/50 shadow-2xl shadow-rose-200 scale-[1.01]'
                        : 'border-slate-100 hover:border-rose-200 bg-white hover:shadow-xl hover:shadow-slate-200'
                        }`}
                ...
                >
                    <CardHeader className="p-6 md:p-8 pb-4 relative">
                        <div className="flex justify-between items-start mb-4 md:mb-6">
                            ...
                            {tiket.kuota < 10 && (
                                <Badge variant="destructive" className="bg-rose-500 font-bold px-3 md:px-4 py-1 rounded-full animate-pulse text-[10px] md:text-xs">
                                    Sisa {tiket.kuota}
                                </Badge>
                            )}
                        </div>
                        <CardTitle className="text-2xl md:text-3xl font-black tracking-tight text-slate-900 group-hover:text-rose-600 transition-colors uppercase italic">{tiket.nama_tiket}</CardTitle>
                    ...
                    <CardContent className="p-6 md:p-8 pt-4">
                        <div className="flex flex-col gap-1">
                            ...
                            <div className="flex items-baseline gap-2">
                                <span className={`text-3xl sm:text-4xl md:text-5xl font-black transition-colors ${selectedTiket?.id === tiket.id ? 'text-rose-600' : 'text-slate-900'}`}>
                    ...
                    <CardFooter className="p-6 md:p-8 pt-0 flex items-center text-[10px] md:text-sm text-slate-400 font-bold uppercase tracking-widest">
                        ...
                        <Clock className="w-3 h-3 md:w-4 md:h-4 mr-2 text-rose-500" />
